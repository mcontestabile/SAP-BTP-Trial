...
CLASS ltcl_integration_test IMPLEMENTATION.
  ...
  METHOD create_travel.
    ...
    DATA travels_in TYPE TABLE FOR CREATE zi_exercise3_travel_U\\travel.
    ...
    MODIFY ENTITIES OF zi_exercise3_travel_U
      ENTITY travel
    ...
    SELECT * FROM zi_exercise3_travel_U WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
  ...
  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities = VALUE #( ( i_for_entity = 'zi_exercise3_travel_U' )
                                ( i_for_entity = 'zi_exercise3_travel_U' ) )
                              ).
  ENDMETHOD.
